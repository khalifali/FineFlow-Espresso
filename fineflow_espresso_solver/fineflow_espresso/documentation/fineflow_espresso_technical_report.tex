...
\graphicspath{{../closure_comparison/}{../closure_comparison/darcy_exponential/}{../verification_results/}}
...
\vfill
{\normalsize Revision: 11 September 2026; Darcy baseline and controlled closure comparison\par}
\vspace{0.3cm}
...
\textbf{Reverse flow and radial channeling are outside the scope of this work.}
Local permeability recovery describes deposit removal, not radial bypass flow.
...
\begin{itemize}
  ...
  \item Darcy flow (default), with optional Forchheimer inertia, is evaluated as an algebraic
...
The exponential is an empirical demonstrator choice, not a coffee-specific
law derived from measurements. Writing $x=m_{\mathrm d}/m_{\mathrm{cap}}$, a
constant logarithmic sensitivity $\mathrm d\ln(K/K_0)/\mathrm dx=-\beta$
integrates to Eq.~\eqref{eq:kexp} with $K/K_0=1$ at $x=0$. This motivates a
smooth, positive and tunable closure; it does not establish its physical validity.
At capacity, the illustrative $\beta=5.5$ imposes $K/K_0=e^{-5.5}=0.00409$
before clipping. Strong blockage is thus an input assumption.
...
\end{equation}
subject to clipping between the configured minimum ratio and one. This is a
relative porosity law with independently prescribed $K_0(z)$. It assumes the
surface/geometry prefactor is unchanged during deposition. A constant Kozeny
factor cancels when dividing by the initial state; the implementation has no
separate Kozeny-factor input. The exponential parameter
\code{permeability\_blocking\_factor} is not a Kozeny factor and is unused here.
Deposit density determines porosity loss, while deposit capacity still limits
capture. Preferential throat blockage and changing specific surface are not
resolved by this relative law.
...
table endpoint and are then clipped between the minimum permeability ratio and one.

\section{Hydraulics, machine control, and basket sieve}

\subsection{Darcy baseline and optional Forchheimer resistance}
...
resistance, with Forchheimer coefficient $\beta_F$ in m$^{-1}$. The default
...
a scalar quadratic equation when inertia is enabled, or a linear resistance
inversion for Darcy flow.

\subsection{Measured permeability and the role of Ergun}
...
\end{equation}
Darcy is selected for the current tested regime. The optional inertial term
should be enabled when pressure--flow data require it. The comparison below
is not evidence that inertia is negligible for every grind or flow rate.
...
\item[Constant mode] The default fixes total pressure without a pulse. Flow follows the current resistance without a pump cap.
...
selected resistance law gives $Q$ (linear for Darcy, quadratic with inertia). An optional pressure pulse can
...
\end{equation}
Advection is first-order upwind and dispersion is centred. Each internal face
has one flux used with opposite signs in its adjacent cells, so internal fluxes
cancel when summing the balances. Equal-and-opposite inventory transfers and
consistent outlet/basket accounting preserve total fine mass. This is why FVM
is a natural conservative choice; conservation is not guaranteed merely by
calling a scheme finite volume. It also does not guarantee accuracy or physical
validity. The implementation monitors the global mass residual;
 the upwind part is monotone under the applied CFL restriction and
...
\section{Controlled comparison and demonstration results}

The frozen \code{comparison\_reference.json} defines a 40-cell, 60-s study.
From the solver directory, reproduce its three cases with:
\begin{verbatim}
python3 compare_closures.py
\end{verbatim} The script does not read the active named case configuration.
All kinetics, initial permeability, controller and numerical settings are held
fixed; no parameters are fitted. Only the indicated switch changes:
\begin{enumerate}
\item Forchheimer + exponential: archived reference configuration.
\item Darcy + exponential: remove inertia only, relative to case 1.
\item Darcy + Kozeny--Carman: change permeability law only, relative to case 2.
\end{enumerate}
The script checks these parameter differences, global mass error and mobile
positivity. The active \code{fine\_puck} case and Python hydraulic default now
use Darcy. Exponential remains the permeability default. The explicit
\code{coarse\_puck} configuration retains its earlier hydraulic selection.

\subsection{Removing inertia has negligible effect in this test}

The maximum flow difference between cases 1 and 2 is
$2.9642\times10^{-7}$ mL/s, or $2.9102\times10^{-7}$ of the reference peak flow.
The maximum total-pressure difference is 0.02034 Pa. These results support
using Darcy for this illustrative regime. They do not establish a universal
absence of inertial losses in espresso pucks.

\subsection{Permeability choice strongly changes the prediction}

\begin{table}[H]
\centering
\caption{Controlled Darcy comparison at 60 s, with no parameter refitting.}
\label{tab:result}
\begin{tabular}{@{}lrr@{}}
\toprule
Quantity & Exponential & Kozeny--Carman\\
\midrule
Final total pressure [bar] & 8.99061 & 9.00031\\
Final flow [mL/s] & 0.04535 & 0.89243\\
Cumulative cup fines [g] & 0.03993 & 0.34664\\
Minimum local $K/K_0$ & 0.01263 & 0.72264\\
\bottomrule
\end{tabular}
\end{table}

The PI controller approaches the same pressure target in both cases, but their
flow and fines predictions differ substantially. The exponential law imposes
strong permeability loss; the porosity-only relative law predicts a milder loss
for this deposit density. This is sensitivity evidence, not validation of
either closure. Do not tune Kozeny--Carman to reproduce the exponential result
as though that result were experimental truth. CT, PNM and independent
pressure--flow and fines measurements must constrain the closure.

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{comparison_flow.pdf}
\caption{Common-input comparison of the predicted flow histories.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{comparison_permeability.pdf}
\caption{Final relative permeability: closure choice dominates the inertia
switch in this test. Initial deposits are zero, so initial permeability equals
$K_0$.}
\end{figure}

\subsection{Comparison metrics and archived outputs}

Metrics in \code{comparison\_metrics.json} compare matching saved times and,
for fields, all axial cells. For a reference array $a$ and candidate $b$,
\[
 E_{\mathrm{abs}}=\max|a-b|,\qquad
 E_{\mathrm{peak}}=\frac{\max|a-b|}{\max|a|}.
\]
The normalized value is a fraction, not a percentage. If the reference peak
is zero, the script records a null normalized value. Peak normalization avoids
pointwise division by zero at startup. These metrics are neither RMSE nor
experimental errors. Parameter snapshots, CSV histories, NPZ profiles and
single-axis PDF/PNG comparison figures are committed with the study.
The \code{results/} directory now contains the constant-pressure baseline.
The closure comparison above retains its original PI settings.
...
program. The archived verification evidence reports 15 unit tests and 17 extended
checks. These checks are numerical verification, not physical validation;
re-run them after solver changes.
...
\begin{itemize}
  ...
  \item Reverse flow and radial channeling are outside scope; cracks and individual basket holes are unresolved.
...
\clearpage
\section{Constant pressure and optional PI control: updated study}
The default operating mode is now \texttt{constant}. It imposes
\texttt{pressure\_drop\_pa} across puck plus basket at every time step:
\[
Q(t)=\frac{\Delta p_{\mathrm{total}}}{R_{\mathrm{puck}}(t)+R_s},\qquad
R_{\mathrm{puck}}(t)=\frac{\mu}{A}\int_0^L\frac{\mathrm dz}{K(z,t)}.
\]
The downstream pressure reference is zero. Pressure at the puck exit is
$QR_s$. Set $R_s=0$ when prescribing the pressure drop across the puck alone.
Constant mode ignores pulse settings and does not enforce a pump-flow cap.
The existing \texttt{pi} mode retains capped flow actuation, pump lag and
conditional integration. Legacy \texttt{pressure} mode retains its rectangular
pulse. An arbitrary measured pressure history is not yet implemented.

\subsection{Matched numerical comparison}
For each puck, only the control mode changes. Both use Darcy with exponential
permeability. The fine-puck duration is 60 s; the coarse-puck duration is 20 s.
\begin{center}
\begin{tabular}{lrrr}\toprule
Case & Pressure [bar] & Flow [mL/s] & Cup fines [g]\\\midrule
fine\_puck\_constant & 9.0000 & 0.04471 & 0.04023\\
fine\_puck\_pi & 8.9906 & 0.04535 & 0.03993\\
coarse\_puck\_constant & 9.0000 & 7.43194 & 0.05147\\
coarse\_puck\_pi & 9.0222 & 7.77635 & 0.04993\\
\bottomrule\end{tabular}\end{center}
The coarse constant-pressure case initially requires 105.02 mL/s. PI is
limited to 10.83 mL/s and reaches only 1.66 bar at 2 s. Deposits subsequently
increase resistance enough for the target to become attainable. This case
illustrates temporary saturation, not sustained sub-target pressure.
The large ideal flow is not a prediction of the actual machine.

\begin{center}
\includegraphics[width=.78\linewidth]{../pressure_comparison/coarse_puck_hydraulics.pdf}
\end{center}

The earlier closure comparison in this report retains its original PI
settings. The updated files in \texttt{results/} use constant pressure.
Complete paired data, parameter snapshots and figures are in
\texttt{pressure\_comparison/}; reproduce them with
\texttt{python3 compare\_pressure\_modes.py} from the solver directory.
All 17 unit tests pass. Maximum relative fines-mass error in the four new runs
is below $2.1\times10^{-15}$.

\subsection{Experimental use}
A nine-bar simulation applies only to the corresponding measured pressure
conditions. For a lower approximately steady pressure, prescribe that value.
Changing experimental pressure requires a history extension or an independently
calibrated machine model. PI alone does not make the comparison valid.
The model assumes a saturated puck and does not describe initial wetting.
\end{document}
